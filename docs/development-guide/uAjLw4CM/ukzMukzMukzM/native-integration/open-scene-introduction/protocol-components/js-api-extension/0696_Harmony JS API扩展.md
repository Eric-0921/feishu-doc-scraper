...
> Source: https://open.feishu.cn/document/uAjLw4CM/ukzMukzMukzM/native-integration/open-scene-introduction/protocol-components/js-api-extension/harmony-js-api-extension

---

# alchemy_customapi_api - Harmony 自定义 API 能力 - 能力组件

| 组件名称 | 组件类型 | 组件版本 |  生效基线 |
| --- | --- | --- | --- |
| alchemy_customapi_api | 能力组件 | 1.0.0 | 7.32.0 |
...
export class KARuntimeManager {
  private customApiPlugin?: IAlchemyCustomApiPlugin;

public loadModule(runtime: TrimisContainerWrapperInterface) {
    this.customApiPlugin = runtime.getCustomApiPlugin();

// 示例：处理自定义 API 调用
    this.customApiPlugin?.handleEvent('getUserInfo', (result) => {
      if (result.resultType === CustomApiResultType.SUCCESS) {
        console.info('API调用成功:', JSON.stringify(result.resultData));
      }
    }, { userId: '12345' });
  }
}
