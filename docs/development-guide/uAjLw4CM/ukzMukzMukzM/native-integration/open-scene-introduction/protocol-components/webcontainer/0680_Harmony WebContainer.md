# Harmony WebContainer

> Source: https://open.feishu.cn/document/uAjLw4CM/ukzMukzMukzM/native-integration/open-scene-introduction/protocol-components/webcontainer/harmony-webcontainer

---

# alchemy_webcontainer_api - Harmony 网页容器能力 - 能力组件

| 组件名称 | 组件类型 | 组件版本 | 生效基线 |
| --- | --- | --- | --- |
| alchemy_webcontainer_api | 能力组件 | 1.0.0 | 7.32.0 |
...
export class KARuntimeManager {
  private alchemyWebContainer?: IAlchemyWebContainer;

public loadModule(runtime: TrimisContainerWrapperInterface) {
    this.alchemyWebContainer = runtime.getAlchemyWebContainer();

// 示例：网页容器即将打开
    this.alchemyWebContainer?.onOpen(
      'https://example.com',
      1001,
      new class implements AlchemyWebContainerCallback {
        onSuccess() {
          console.info('内部流程处理完成，继续加载网页');
        }
        onFail(code: number) {
          console.error(`内部流程处理失败，错误码：${code}`);
        }
      }
    );

// 示例：获取错误页配置
    const errorConfig = this.alchemyWebContainer?.getErrorPageConfig();
    console.info('错误页配置：', errorConfig);
...
// 示例：Tab 可见性变化
    const tabParams: TabParams = {
      ...
      appId: 'app123',
      componentUniqueId: 1001
    };
    this.alchemyWebContainer?.onTabVisibilityChanged(true, tabParams);
  }
}
```
