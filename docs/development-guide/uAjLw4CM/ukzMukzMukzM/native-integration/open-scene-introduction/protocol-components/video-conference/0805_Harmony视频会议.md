...
> Source: https://open.feishu.cn/document/uAjLw4CM/ukzMukzMukzM/native-integration/open-scene-introduction/protocol-components/video-conference/harmony-video-conference

---

# alchemy_vc_api - Harmony 视频通话能力 - 能力组件

| 组件名称 | 组件类型 | 组件版本 | 生效基线 |
| --- | --- | --- | --- |
| alchemy_vc_api | 能力组件 | 1.0.0 | 7.32.0 |
...
export class KARuntimeManager {
  private alchemyVideoChat?: IAlchemyVideoChat;

public loadModule(runtime: TrimisContainerWrapperInterface) {
    this.alchemyVideoChat = runtime.getAlchemyVideoChat();
    this.alchemyVideoChat?.onStart('meeting-123', Type.MEET);
    this.alchemyVideoChat?.onCameraMuted('meeting-123', Type.MEET);
    this.alchemyVideoChat?.onFloatWindowStart('meeting-123', Type.MEET);
    // 其他方法省略...
  }
}
```
