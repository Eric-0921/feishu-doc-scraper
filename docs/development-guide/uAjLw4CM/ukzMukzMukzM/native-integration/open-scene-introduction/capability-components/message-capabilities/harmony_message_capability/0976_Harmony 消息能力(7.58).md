...
> Source: https://open.feishu.cn/document/uAjLw4CM/ukzMukzMukzM/native-integration/open-scene-introduction/capability-components/message-capabilities/harmony_message_capability/harmony-message-capability-758
...
interface IMessageAPI {
  forward(uniqueId: number, message: Message): void // 转发消息到聊天
  getMessageResource(actionMessage: ActionMessage): Promise<MessageInfo | undefined> // 获取消息相关信息
  downloadResource(messageInfo: MessageInfo): Promise<Message | undefined> // 通过消息信息下载消息资源，失败时返回undefined
}
```
...
| 参数名称 | 类型 | 必填 | 描述 |
| --- | --- | --- | --- |
| uniqueId | number | 是 | 当前组件的 UniqueId，一般可由 this.getUniqueId() 获取 |
...
| 参数名称 | 类型 | 必填 | 描述 |
| --- | --- | --- | --- |
| actionMessage | ActionMessage | 是 | 消息类型和 ID |

**返回值**：消息信息或 undefined

#### `downloadResource(messageInfo: MessageInfo): Promise<Message | undefined>`
通过消息信息下载消息资源，失败时返回 undefined
