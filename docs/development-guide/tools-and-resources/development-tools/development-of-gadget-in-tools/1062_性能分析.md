...
| AppLifeCycle | App.onLaunch<br>App.onShow<br>App.onHide<br>App.onUnlaunch | 开发者通过 [App](https://open.feishu.cn/document/uYjL24iN/uMDNuMDNuMDN) 方法注册的 App 实例的生命周期回调函数。|
| AppMethod | 按照格式 `App.${methodName}`，methodName 为自定义方法的方法名 | 通过 [App](https://open.feishu.cn/document/uYjL24iN/uMDNuMDNuMDN) 方法注册的 App 实例的自定义方法 |
...
| PageLifeCycle | Page.onLoad<br>Page.onReady<br>Page.onShow<br>Page.onHide<br>Page.onUnload<br>Page.onReload | 通过 [Page](https://open.feishu.cn/document/uYjL24iN/uQDNuQDNuQDN) 方法注册的 Page 实例的生命周期回调函数 |
| PageMethod | 按照格式 `Page.${methodName}`，methodName 为自定义方法的方法名 | 通过 [Page](https://open.feishu.cn/document/uYjL24iN/uQDNuQDNuQDN) 方法注册的 Page 实例的自定义方法 |
| ComponentLifeCycle | Component.created<br>Component.ready<br>Component.attached<br>Component.detached | 通过 [Component](https://open.feishu.cn/document/uYjL24iN/uADMx4CMwEjLwATM) 方法注册的 Component 实例的生命周期回调函数 |
| ComponentMethod | 按照格式 `Component.${methodName}`，methodName 为自定义方法的方法名 | 通过 [Component](https://open.feishu.cn/document/uYjL24iN/uADMx4CMwEjLwATM) 方法注册的 Componet 实例的自定义方法 |
| ComponentObserver | 按照格式 `Component.${key}.observer`，key 为自定义组件属性名 | 通过 [Component](https://open.feishu.cn/document/uYjL24iN/uADMx4CMwEjLwATM) 方法注册的 Componet 实例属性变化时触发的回调函数 |
