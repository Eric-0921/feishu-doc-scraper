...
behaviors | 否 | Array | 空 | 当用户点击折叠按钮组内的任意按钮后，除按钮本身的回传数据以外，还可通过 behaviors 参数回传更多数据。配置示例如下：<br>```json<br>{<br>"behaviors": [<br>{<br>"type": "callback", // 声明交互类型是回传数据到服务端的请求回调交互。<br>"value": {<br>// 回传交互数据。支持 object 数据类型。开放平台 SDK 仅支持 object 类型的回传交互数据。<br>"key_1": "value_1" // 示例数据。<br>}<br>}<br>]<br>}<br>```
