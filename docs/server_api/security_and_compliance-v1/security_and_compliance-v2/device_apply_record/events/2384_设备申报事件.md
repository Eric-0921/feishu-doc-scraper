...
> Source: https://open.feishu.cn/document/security_and_compliance-v1/security_and_compliance-v2/device_apply_record/events/device_apply_event
...
支持的应用类型 | Custom App
权限要求<br>**订阅该事件所需的权限，开启其中任意一项权限即可订阅** | 查询自主申报申请(security_and_compliance:device_apply_record:read)
...
apply_status | int | 申报状态<br>**可选值有**：<br>- 0：未知<br>- 1：待审核<br>- 2：审核通过<br>- 3：已驳回
operator | user_id | 用户 ID
union_id | string | 用户的 union id
user_id | string | 用户的 user id<br>**字段权限要求**：<br>获取用户 user ID(contact:user.employee_id:readonly)
open_id | string | 用户的 open id
apply_device_ownership | int | 申报设备归属<br>**可选值有**：<br>- 0：未知设备<br>- 1：个人设备<br>- 2：企业设备
