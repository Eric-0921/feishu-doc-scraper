...
> Source: https://open.feishu.cn/document/apaas-v1/seat_assignment/list
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用** | 查询席位分配详情(app_engine:seat_assignments:read)
...
名称 | 类型 | 必填 | 描述
---|---|---|---
seat_type | string | 是 | 席位类型，枚举值：per_user、per_user_per_app<br>**示例值**：per_user<br>**可选值有**：<br>- per_user：平台席位<br>- per_user_per_app：应用访问席位
...
status | string | 席位状态，枚举值：in_use 、released<br>**可选值有**：<br>- in_use：席位生效中<br>- released：席位已释放
