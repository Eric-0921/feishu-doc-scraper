...
> Source: https://open.feishu.cn/document/apaas-v1/permission/application-role-member/get
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用** | 查询角色配置信息(app_engine:role:read)
...
id | string | id
name | string | 名称
type | string | 角色成员模式<br>**可选值有**：<br>- all：全部用户<br>- custom： 自定义
