...
> Source: https://open.feishu.cn/document/apaas-v1/application-object-record/patch
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用** | 创建、更新、删除对象记录数据(app_engine:object.record:write)
...
名称 | 类型 | 必填 | 描述
---|---|---|---
record | string | 是 | 创建对象使用的数据，键为字段 API 名称，值为字段值，格式可参考字段值格式<br>**示例值**："{\"_id\":\"1798681438085228\",\"book_count\":\"3\",\"book_name\":\"showcase2\"}"<br>**数据校验规则**：<br>- 最小长度：`0` 字符
