...
支持的应用类型 | Custom App
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用** | 更新用户 ID(contact:contact:update_user_id)
...
名称 | 类型 | 描述
---|---|---
user_id | string | 用户 ID，ID 类型与查询参数 user_id_type 的取值保持一致。<br>**示例值**："ou-938e3e4fdc5e1993bee01250076f0cc2"
...
名称 | 类型 | 必填 | 描述
---|---|---|---
new_user_id | string | 是 | 自定义新的用户 user_id。长度不能超过 64 字符。<br>**示例值**："3e3cf96b"
