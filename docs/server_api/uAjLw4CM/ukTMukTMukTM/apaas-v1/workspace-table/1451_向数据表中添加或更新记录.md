...
> Source: https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/apaas-v1/workspace-table/records_post
...
支持的应用类型 | Custom App
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用** | 修改或删除数据表数据记录(app_engine:workspace.table.record:write)
...
Prefer | string | 否 | 为空时表示 INSERT，<br>resolution=merge-duplicates表示 UPSERT，详细参考 https://docs.postgrest.org/en/v13/references/api/tables_views.html#upsert<br>**示例值**："resolution=merge-duplicates"
...
on_conflict | string | 否 | UPSERT 时使用，指定使用哪一个或多个具有唯一约束的字段作为冲突判断依据，默认为表主键。<br>假设 user_products 表有一个由 user_id 和 product_id 组成的复合唯一约束。<br>**示例值**：user_id,product_id

### 请求体

名称 | 类型 | 必填 | 描述
---|---|---|---
records | string | 是 | 要插入的数据记录列表，单次支持最多 500 条<br>**示例值**："[{\"name\":\"王一一\",\"gender\":\"male\",\"age\":10},{\"name\":\"王二二\",\"gender\":\"female\",\"age\":10}]"
