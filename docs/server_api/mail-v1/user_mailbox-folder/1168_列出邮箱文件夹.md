...
> Source: https://open.feishu.cn/document/mail-v1/user_mailbox-folder/list
...
支持的应用类型 | Custom App
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 查询邮箱文件夹(mail:user_mailbox.folder:read)<br>查看、创建、更新、删除邮箱文件夹(mail:user_mailbox.folder:write)
...
名称 | 类型 | 必填 | 描述
---|---|---|---
folder_type | int | 否 | 文件夹类型<br>**示例值**：1<br>**可选值有**：<br>- 1：系统文件夹<br>- 2：用户文件夹<br>**数据校验规则**：<br>- 取值范围：`1` ～ `2`
