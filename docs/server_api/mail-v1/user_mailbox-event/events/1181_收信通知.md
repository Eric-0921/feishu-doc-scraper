...
> Source: https://open.feishu.cn/document/mail-v1/user_mailbox-event/events/message_received
...
支持的应用类型 | Custom App
权限要求<br>**订阅该事件所需的权限，开启其中任意一项权限即可订阅** | 订阅邮箱相关事件(mail:event)
字段权限要求 | **注意事项**：该接口返回体中存在下列敏感字段，仅当开启对应的权限后才会返回；如果无需获取这些字段，则不建议申请<br>获取事件中的邮箱地址字段(mail:user_mailbox.event.mail_address:read)
...
event | \- | \-
mail_address | string | 收信的邮箱<br>**字段权限要求**：<br>获取事件中的邮箱地址字段(mail:user_mailbox.event.mail_address:read)
message_id | string | 邮件 id
mailbox_type | int | 收到邮件的邮箱类型<br>**可选值有**：<br>- 1：个人邮箱<br>- 2：公共邮箱<br>**数据校验规则**：<br>- 取值范围：`1` ～ `2`
