...
> Source: https://open.feishu.cn/document/docs/docs/document-block/create-2
...
client_token | string | 否 | 操作的唯一标识。**UUIDv4** 格式，与接口返回值的 client_token 相对应，用于幂等的进行更新操作。此值为空表示将发起一次新的请求，此值非空表示幂等的进行更新操作<br>**示例值**："8aac2291-bc9e-4b12-a162-b3cf15bb06bd"
...
url | string | 是 | 超链接指向的 url (需要 url_encode)<br>**示例值**："https%3A%2F%2Fopen.feishu.cn%2F"
comment_ids | string\[\] | 否 | 评论 ID 列表。在创建 Block 时，不支持传入评论 ID；在更新文本 Block 的 Element 时，允许将对应版本已存在的评论 ID 移动到同一个 Block 内的任意 Element 中，但不支持传入新的评论 ID。如需查询评论内容请阅览「[获取回复](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/drive-v1/file-comment-reply/list)」 API。<br>**示例值**：["1660030311959965796"]
bitable | bitable | 否 | 多维表格 Block
view_type | int | 否 | 类型<br>**示例值**：1<br>**可选值有**：<br>- 1：数据表<br>- 2：看板
callout | callout | 否 | 高亮块 Block
background_color | int | 否 | 高亮块背景色<br>**示例值**：1<br>**可选值有**：<br>- 1：浅红色<br>- 2：浅橙色<br>- 3：浅黄色<br>- 4：浅绿色<br>- 5：浅蓝色<br>- 6：浅紫色<br>- 7：中灰色<br>- 8：中红色<br>- 9：中橙色<br>- 10：中黄色<br>- 11：中绿色<br>- 12：中蓝色<br>- 13：中紫色<br>- 14：灰色<br>- 15：浅灰色
border_color | int | 否 | 边框色<br>**示例值**：1<br>**可选值有**：<br>- 1：红色<br>- 2：橙色<br>- 3：黄色<br>- 4：绿色<br>- 5：蓝色<br>- 6：紫色<br>- 7：灰色
text_color | int | 否 | 文字颜色<br>**示例值**：1<br>**可选值有**：<br>- 1：红色<br>- 2：橙色<br>- 3：黄色<br>- 4：绿色<br>- 5：蓝色<br>- 6：紫色<br>- 7：灰色
...
file | file | 否 | 文件 Block。了解如何在文档中插入文件，参考[文档常见问题-如何插入文件/附件](https://open.feishu.cn/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/faq)。
view_type | int | 否 | 视图类型，卡片视图（默认）或预览视图<br>**示例值**：1<br>**可选值有**：<br>- 1：卡片视图<br>- 2：预览视图
grid | grid | 否 | 分栏 Block
column_size | int | 是 | 分栏列数量<br>**示例值**：2<br>**数据校验规则**：<br>- 取值范围：`2` ～ `5`
grid_column | grid_column | 否 | 分栏列 Block
iframe | iframe | 否 | 内嵌 Block
component | iframe_component | 是 | iframe 的组成元素
iframe_type | int | 否 | iframe 类型<br>**示例值**：1<br>**可选值有**：<br>- 1：哔哩哔哩<br>- 2：西瓜视频<br>- 3：优酷<br>- 4：Airtable<br>- 5：百度地图<br>- 6：高德地图<br>- 7：Undefined<br>- 8：Figma<br>- 9：墨刀<br>- 10：Canva<br>- 11：CodePen<br>- 12：飞书问卷<br>- 13：金数据<br>- 14：Undefined<br>- 15：Undefined<br>- 99：Other
url | string | 是 | iframe 目标 url（需要进行 url_encode）<br>**示例值**："https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV1Hi4y1w7V7"
image | image | 否 | 图片 Block。了解如何在文档中插入图片，参考[文档常见问题-如何插入图片](https://open.feishu.cn/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/faq)。
align | int | 否 | 对齐方式<br>**示例值**：2<br>**可选值有**：<br>- 1：居左排版<br>- 2：居中排版<br>- 3：居右排版<br>**默认值**：`2`
caption | caption | 否 | 图片描述
content | string | 否 | 描述的文本内容<br>**示例值**："caption"
isv | isv | 否 | 三方 Block
add_ons | add_ons | 否 | Add-ons
component_id | string | 否 | 文档小组件 ID。该 ID 可通过调用[创建 BlockEntity](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/block-v2/entity/create) 接口，并从响应体中的 block_id 中获取，创建时使用的 `block_type_id` 需要与 `component_type_id` 一致。<br>**示例值**："7056882725002051603"
component_type_id | string | 是 | 文档小组件类型，比如问答互动"blk_636a0a6657db8001c8df5488"。该 ID 可在 [开发者后台](https://open.feishu.cn/app) > **应用详情页** > **应用能力** > **云文档小组件** > **BlockTypeID** 获取。<br>**示例值**："blk_636a0a6657db8001c8df5488"
record | string | 否 | 文档小组件内容数据，JSON 字符串<br>**示例值**：""{}""
sheet | sheet | 否 | 电子表格 Block
row_size | int | 否 | 电子表格行数量<br>**示例值**：2<br>**数据校验规则**：<br>- 取值范围：`1` ～ `9`
column_size | int | 否 | 电子表格列数量<br>**示例值**：2<br>**数据校验规则**：<br>- 取值范围：`1` ～ `9`
table | table | 否 | 表格 Block
property | table_property | 是 | 表格属性
row_size | int | 是 | 行数<br>- **创建块**接口中，该字段最大值为 9 <br>- **创建嵌套块**接口中，在单个表格单元格不超过上限 2000 情况下，该字段无固定最大值<br>**示例值**：1<br>**数据校验规则**：<br>- 最小值：`1`
column_size | int | 是 | 列数<br>- **创建块**接口中，该字段最大值为 9 <br>- **创建嵌套块**接口中，该字段最大值为 100<br>**示例值**：1<br>**数据校验规则**：<br>- 最小值：`1`
column_width | int\[\] | 否 | 列宽，单位像素（px）<br>**示例值**：[100]<br>**默认值**：`100`
merge_info | table_merge_info\[\] | 否 | 单元格合并信息。创建 Table 时，此属性只读，将由系统自动生成。如果需要合并单元格，可以通过更新块接口的子请求 `merge_table_cells` 实现
row_span | int | 否 | 从当前行索引起被合并的连续行数<br>**示例值**：2<br>**数据校验规则**：<br>- 最小值：`1`
col_span | int | 否 | 从当前列索引起被合并的连续列数<br>**示例值**：2<br>**数据校验规则**：<br>- 最小值：`1`
header_row | boolean | 否 | 设置首行为标题行<br>**示例值**：false<br>**默认值**：`false`
header_column | boolean | 否 | 设置首列为标题列<br>**示例值**：false<br>**默认值**：`false`
table_cell | table_cell | 否 | 单元格 Block
view | view | 否 | 视图 Block
quote_container | quote_container | 否 | 引用容器 Block。为空结构体，需传入 `{}` 创建引用容器 Block。
task | task | 否 | 任务 Block
folded | boolean | 否 | 折叠状态<br>**示例值**：false<br>**默认值**：`false`
okr | okr | 否 | OKR Block，仅可在使用 `user_access_token` 时创建
okr_id | string | 否 | OKR ID，获取需要插入的 OKR ID 可见[获取用户的 OKR 列表](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/okr-v1/user-okr/list)<br>**示例值**："7076349900476448796"
objectives | objective_id_with_kr_id\[\] | 否 | OKR Block 中的 Objective ID 和 Key Result ID，此值为空时插入 OKR 下所有的 Objective 和 Key Result
objective_id | string | 否 | OKR 中 Objective 的 ID<br>**示例值**："7109022409227026460"
kr_ids | string\[\] | 否 | Key Result 的 ID 列表，此值为空时插入当前 Objective 下的所有 Key Result<br>**示例值**：["7109022573011894300","7109022546444517404"]
comment_ids | string\[\] | 否 | 评论 id 列表<br>**示例值**：[1660030311959965796]
wiki_catalog | wiki_catalog | 否 | Wiki 子目录 Block
wiki_token | string | 否 | 知识库 token<br>**示例值**："Ub47wVl7AikG9wkgnpSbFy4EcAc"
board | board | 否 | 画板 Block
align | int | 否 | 对齐方式<br>**示例值**：2<br>**可选值有**：<br>- 1：居左排版<br>- 2：居中排版<br>- 3：居右排版<br>**默认值**：`1`
width | int | 否 | 宽度，单位 px；不填时自动适应文档宽度；值超出文档最大宽度时，页面渲染为文档最大宽度<br>**示例值**：300<br>**数据校验规则**：<br>- 最小值：`90`
height | int | 否 | 高度，单位 px；不填时自动根据画板内容计算；值超出屏幕两倍高度时，页面渲染为屏幕两倍高度<br>**示例值**：300<br>**数据校验规则**：<br>- 最小值：`90`
link_preview | link_preview | 否 | 链接预览 Block
url | string | 是 | 链接<br>**示例值**："https://applink.feishu.cn/client/message/link/open?token=Al7F******Q%3D"<br>**数据校验规则**：<br>- 最小长度：`1` 字符
url_type | string | 是 | 链接类型<br>**示例值**："MessageLink"<br>**可选值有**：<br>- MessageLink：消息链接<br>- Undefined：未定义的链接类型
source_synced | source_synced | 否 | 源同步块，仅支持查询
elements | text_element\[\] | 否 | 同步块独立页标题，由文本元素组成
text_run | text_run | 否 | 文字。支持对 Page、Text、Heading1~9、Bullet、Ordered、Code、Quote、Todo 块进行修改
content | string | 是 | 文本内容。要实现文本内容的换行，你可以：<br>- 在传入的文本内容中添加 `\n` 实现软换行（Soft Break，与在文档中通过操作 `Shift + Enter` 的效果一致）<br>- 创建一个新的文本 Block，实现两个文本 Block 之间的硬换行（Hard Break，与在文档中通过操作 `Enter` 的效果一致）<br>**注意**：软换行在渲染时可能会被忽略，具体取决于渲染器如何处理；硬换行在渲染时始终会显示为一个新行。<br>**示例值**："文本"
text_element_style | text_element_style | 否 | 文本局部样式
bold | boolean | 否 | 加粗<br>**示例值**：true<br>**默认值**：`false`
...
url | string | 是 | 超链接指向的 url (需要 url_encode)<br>**示例值**："https%3A%2F%2Fopen.feishu.cn%2F"
comment_ids | string\[\] | 否 | 评论 ID 列表。在创建 Block 时，不支持传入评论 ID；在更新文本 Block 的 Element 时，允许将对应版本已存在的评论 ID 移动到同一个 Block 内的任意 Element 中，但不支持传入新的评论 ID。如需查询评论内容请阅览「[获取回复](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/drive-v1/file-comment-reply/list)」 API。<br>**示例值**：["1660030311959965796"]
align | int | 否 | 对齐方式<br>**示例值**：1<br>**可选值有**：<br>- 1：居左排版<br>- 2：居中排版<br>- 3：居右排版
reference_synced | reference_synced | 否 | 引用同步块，仅支持查询。获取引用同步块内容详见：[如何获取引用同步块的内容](https://open.feishu.cn/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/faq#19b71234)
source_document_id | string | 否 | 源文档的文档 ID<br>**示例值**："doxcnSS4ouQkQEouGSUkTgabcef"
source_block_id | string | 否 | 源同步块的 Block ID<br>**示例值**："doxcnePuYufKa49ISjhD8Iabcef"
sub_page_list | sub_page_list | 否 | Wiki 新版子目录
wiki_token | string | 是 | 知识库节点 token，仅支持知识库文档创建子页面列表，且需传入当前页面的 wiki token<br>**示例值**："Ub47wVI7AikG9wkgnpSbFyabcef"<br>**数据校验规则**：<br>- 长度范围：`27` ～ `27` 字符
ai_template | ai_template | 否 | AI 模板 Block，仅支持查询
...
document_revision_id | int | 当前提交的 Block 创建成功后文档的版本号
client_token | string | 操作的唯一标识，更新请求中使用此值表示幂等的进行此次更新
block_id_relations | block_id_relation\[\] | 传入的临时 Block ID 与实际 Block ID 映射关系
temporary_block_id | string | 你传入的自定义临时 Block ID
block_id | string | 实际的 Block ID
...
400 | 1770041 | open schema mismatch | 确认块父子关系是否合法。<br>文档场景中，可参考[文档概述-基本概念](https://open.feishu.cn/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-overview)了解块的父子关系规则。
