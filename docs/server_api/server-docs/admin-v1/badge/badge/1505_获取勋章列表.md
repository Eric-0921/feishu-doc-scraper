...
> Source: https://open.feishu.cn/document/server-docs/admin-v1/badge/badge/list
...
page_token | string | 否 | 分页标记，第一次请求不填，表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token，下次遍历可采用该 page_token 获取查询结果<br>**示例值**："b1fmUz"<br>**数据校验规则**：<br>- 长度范围：`1` ～ `64` 字符
name | string | 否 | 租户内唯一的勋章名称，精确匹配。<br>**示例值**："激励勋章"<br>**数据校验规则**：<br>- 最小长度：`1` 字符
