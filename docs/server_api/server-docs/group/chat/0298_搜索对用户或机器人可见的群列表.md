...
> Source: https://open.feishu.cn/document/server-docs/group/chat/search
...
query | string | 否 | 关键词<br>**注意**：<br>- 关键词支持匹配群国际化名称、群成员名称<br>- 支持使用多语种搜索（飞书客户端内支持的多语种）<br>- 支持拼音、前缀等模糊搜索<br>- 关键词为空值或长度超过 `64` 个字符时将返回空的结果<br>- 关键词中尽量不要包含 `-` 符号。如果必须包含该符号，请在传值时添加双引号，例如 `“Example-0”`。<br>**示例值**：abc
...
page_size | int | 否 | 分页大小，用来限制一次请求所返回的数据条目数。<br>**示例值**：10<br>**默认值**：`20`<br>**数据校验规则**：<br>- 最大值：`100`
