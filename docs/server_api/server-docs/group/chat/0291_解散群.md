...
> Source: https://open.feishu.cn/document/server-docs/group/chat/delete
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 获取与更新群组信息(im:chat)<br>解散群(im:chat:delete)
...
400 | 232017 | No Permission: If the operator is NOT owner or creator with the scope, the operator can NOT complete the request. | 调用接口的操作者在群组中不具备群主、群创建者等管理身份，无法操作。<br>- 如果以应用身份（tenant_access_token）解散群，则应用机器人需要符合以下任一情况。<br>- 应用机器人是群主。<br>- 应用机器人是群的创建者，且应用已开通 **更新应用所创建群的群信息**（im:chat:operate_as_owner）权限。<br>- 如果以用户身份（user_access_token）解散群，需要该用户是群主。
