...
> Source: https://open.feishu.cn/document/server-docs/group/chat-tab/sort_tabs
...
名称 | 类型 | 必填 | 描述
---|---|---|---
tab_ids | string\[\] | 否 | 会话标签页 ID 列表，ID 可以在[添加会话标签页](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat-tab/create)或者[拉取会话标签页](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat-tab/list_tabs)接口返回值中获取。<br>**注意**：<br>- 必须包含会话内全部标签页的 ID。<br>- 当前参数的排序，对应会话内从左往右的排序。<br>- 会话内消息类型的标签页固定在第一顺位。<br>**示例值**：["6936075528890826780"]
