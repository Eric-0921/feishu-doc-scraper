...
> Source: https://open.feishu.cn/document/server-docs/group/chat-member/event/added
...
- 你需要在应用中配置事件订阅，在事件列表的 ==消息与群组== 分类下订阅 ==用户进群== 事件，这样才可以在事件触发时接收到事件数据。了解事件订阅可参见[事件订阅概述](https://open.feishu.cn/document/ukTMukTMukTM/uUTNz4SN1MjL1UzM)。

## 事件

基本 | &nbsp;
---|---
事件类型 | im.chat.member.user.added_v1
...
users | chat.member.user\[\] | 被添加的用户列表
name | string | 用户名字
