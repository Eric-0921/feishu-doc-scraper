...
> Source: https://open.feishu.cn/document/server-docs/contact-v3/functional_role-member/batch_create
...
名称 | 类型 | 必填 | 描述
---|---|---|---
members | string\[\] | 是 | 待添加为角色成员的用户 ID 列表，以 `["xxx", "yyy"]` 数组格式进行传值。ID 类型需要和查询参数 user_id_type 的取值保持一致。<br>**示例值**：["ou-12832197382"]<br>**数据校验规则**：<br>- 长度范围：`1` ～ `100`
...
reason | int | 添加成员的处理结果。<br>**可选值有**：<br>- 1：处理成功<br>- 2：用户 ID 无效<br>- 3：无权限操作该用户 ID<br>- 4：用户已存在该角色中<br>- 5：用户不存在该角色中<br>- 6：无权限操作该角色成员旧的管理范围
