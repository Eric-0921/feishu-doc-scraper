...
> Source: https://open.feishu.cn/document/server-docs/contact-v3/job_level/update
...
支持的应用类型 | Custom App
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 更新通讯录(contact:contact)<br>创建、删除、更新职级(contact:job_level)
...
名称 | 类型 | 描述
---|---|---
job_level_id | string | 职级 ID。获取方式：<br>- 创建职级时，可以从返回结果中获取职级 ID。<br>- 调用[获取租户职级列表](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/job_level/list)接口，查找指定职级的 ID 信息。<br>**示例值**："mga5oa8ayjlp9rb"
...
status | boolean | 否 | 是否启用该职级。<br>**可选值有**：<br>- true：启用<br>- false：不启用<br>**默认值**：空，表示不更新。<br>**示例值**：true
i18n_name | i18n_content\[\] | 否 | 多语言职级名称。
locale | string | 否 | 语言版本。例如：<br>- zh_cn：中文<br>- en_us：英语<br>- ja_jp：日语<br>**默认值**：空，表示不更新。<br>**示例值**："zh_cn"
value | string | 否 | 语言版本对应的职级名称。<br>**默认值**：空，表示不更新。<br>**示例值**："多语言内容"
i18n_description | i18n_content\[\] | 否 | 多语言职级描述。
locale | string | 否 | 语言版本。例如：<br>- zh_cn：中文<br>- en_us：英语<br>- ja_jp：日语<br>**示例值**："zh_cn"
...
status | boolean | 是否启用职级。<br>**可能值有**：<br>true：启用<br>false：不启用
