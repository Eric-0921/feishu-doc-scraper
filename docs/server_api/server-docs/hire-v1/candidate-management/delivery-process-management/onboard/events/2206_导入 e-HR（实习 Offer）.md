...
> Source: https://open.feishu.cn/document/server-docs/hire-v1/candidate-management/delivery-process-management/onboard/events/imported
...
**注意事项**：- 需通过 [更新 e-HR 导入任务结果](https://open.feishu.cn/document/ukTMukTMukTM/uMzM1YjLzMTN24yMzUjN/hire-v1/ehr_import_task/patch) 更新本次导入e-HR任务的结果
...
支持的应用类型 | Custom App
权限要求<br>**订阅该事件所需的权限，开启其中任意一项权限即可订阅** | 更新导入 e-HR 任务(hire:ehr_import)
...
offer_id | string | Offer ID，详情请参考[获取 Offer 详情](https://open.feishu.cn/document/ukTMukTMukTM/uMzM1YjLzMTN24yMzUjN/hire-v1/offer/get)
pre_onboard_id | string | 入职ID（实习）
ehr_department_id | string | 导入部门 ID，等同于`ehr_department. department_id`字段
operator_id | string | 操作人的用户user id，等同于`operator_user_id.user_id`字段
operator_user_id | user_id | 操作人用户 ID
...
ehr_department | department_id | 导入部门ID
department_id | string | 导入部门的部门 ID
open_department_id | string | 导入部门的飞书部门 ID
