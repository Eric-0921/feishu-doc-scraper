...
> Source: https://open.feishu.cn/document/server-docs/hire-v1/candidate-management/delivery-process-management/application/create
...
名称 | 类型 | 必填 | 描述
---|---|---|---
talent_id | string | 是 | 人才 ID，可通过[获取人才列表](https://open.feishu.cn/document/ukTMukTMukTM/uMzM1YjLzMTN24yMzUjN/hire-v1/talent/list)获取<br>**示例值**："6889020179456689671"
job_id | string | 是 | 职位 ID，可通过[获取职位列表](https://open.feishu.cn/document/ukTMukTMukTM/uMzM1YjLzMTN24yMzUjN/hire-v1/job/list)获取<br>**示例值**："6889330484657424647"
user_id | string | 否 | 人员 ID，与入参 `user_id_type` 类型一致。<br>**注意**：<br>若投递来源为属于「员工转岗」或「实习生转正」时必填，创建投递成功后会将该员工和对应人才进行绑定；创建其他来源投递时，不会进行人员与人才绑定。<br>**示例值**："6930815272790114324"
resume_source_id | string | 否 | 简历来源 ID，可通过[获取简历来源列表](https://open.feishu.cn/document/ukTMukTMukTM/uMzM1YjLzMTN24yMzUjN/hire-v1/resume_source/list)获取<br>**示例值**："7115289562569591070"
application_preferred_city_code_list | string\[\] | 否 | 意向投递城市列表，可通过[查询地点列表](https://open.feishu.cn/document/ukTMukTMukTM/uMzM1YjLzMTN24yMzUjN/hire-v1/location/query)<br>获取到对应的城市编码<br>**示例值**：["CN_1"]
delivery_type | int | 否 | 投递方式<br>**示例值**：1<br>**可选值有**：<br>- 1：HR 寻访<br>- 2：候选人主动投递
