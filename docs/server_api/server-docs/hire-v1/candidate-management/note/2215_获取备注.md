...
> Source: https://open.feishu.cn/document/server-docs/hire-v1/candidate-management/note/get
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 获取招聘备注信息(hire:note:readonly)<br>更新招聘备注信息(hire:note)
...
名称 | 类型 | 描述
---|---|---
note_id | string | 备注 ID，可通过[获取备注列表](https://open.feishu.cn/document/ukTMukTMukTM/uMzM1YjLzMTN24yMzUjN/hire-v1/note/list)获取<br>**示例值**："6949805467799537964"
...
note | note | 人才备注信息
id | string | 备注 ID
talent_id | string | 人才 ID，详情请参考：[获取人才信息 ](https://open.feishu.cn/document/ukTMukTMukTM/uMzM1YjLzMTN24yMzUjN/hire-v1/talent/get)
...
is_private | boolean | 是否私密备注
create_time | int | 创建时间，毫秒时间戳
modify_time | int | 更新时间，毫秒时间戳
creator_id | string | 创建人 ID，与入参中 `user_id_type` 类型一致
content | string | 备注内容
