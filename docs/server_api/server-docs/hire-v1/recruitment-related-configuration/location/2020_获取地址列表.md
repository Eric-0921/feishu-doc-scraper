...
> Source: https://open.feishu.cn/document/server-docs/hire-v1/recruitment-related-configuration/location/list
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用** | 获取招聘相关位置信息(hire:location:readonly)
...
page_size | int | 否 | 每页获取记录数量<br>**示例值**：100<br>**默认值**：`10`<br>**数据校验规则**：<br>- 最大值：`100`
usage | string | 是 | 地址类型<br>**示例值**：interview_location<br>**可选值有**：<br>- position_location：职位地址<br>- interview_location：面试地址<br>- store_location：门店地址。当前仅支持未接入飞书人事的租户使用
...
zh_cn | string | 国家中文名称
en_us | string | 国家英文名称
active_status | int | 地址启用状态<br>**可选值有**：<br>- 1：已启用<br>- 2：已禁用
