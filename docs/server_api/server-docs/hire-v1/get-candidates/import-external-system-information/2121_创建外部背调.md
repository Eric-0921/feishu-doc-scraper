...
> Source: https://open.feishu.cn/document/server-docs/hire-v1/get-candidates/import-external-system-information/create-2
...
名称 | 类型 | 必填 | 描述
---|---|---|---
external_id | string | 否 | 外部系统背调主键 （仅用于幂等）<br>- 若不传此值，则不进行幂等校验<br>- 若传此值，则用于幂等校验，同一`external_id` 24小时内仅可创建一次<br>**示例值**："7003247299220982060"
...
date | int | 否 | 背调日期，毫秒时间戳（字段类型为：int64）<br>**示例值**：1626602069392
name | string | 否 | 背调名称<br>**示例值**："张三的背调"
result | string | 否 | 背调结果<br>**示例值**："已通过"
attachment_id_list | string\[\] | 否 | 背调附件 ID 列表，可通过[创建附件](https://open.feishu.cn/document/ukTMukTMukTM/uIDN1YjLyQTN24iM0UjN/create_attachment)接口返回<br>**示例值**：["7249929009008494908"]
