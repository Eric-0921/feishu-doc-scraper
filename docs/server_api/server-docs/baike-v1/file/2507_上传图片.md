...
> Source: https://open.feishu.cn/document/server-docs/baike-v1/file/upload
...
名称 | 类型 | 必填 | 描述
---|---|---|---
name | string | 是 | 文件名称，当前仅支持上传图片且图片格式为以下六种：icon、bmp、gif、png、jpeg、webp<br>**示例值**："示例图片.png"<br>**数据校验规则**：<br>- 长度范围：`1` ～ `100` 字符
file | file | 是 | 二进制文件内容，高宽像素在 320-4096 像素之间，大小在 3KB-10MB 的图片<br>**示例值**：file binary
