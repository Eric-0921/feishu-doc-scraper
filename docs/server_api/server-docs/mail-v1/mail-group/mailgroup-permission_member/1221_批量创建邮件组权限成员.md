...
> Source: https://open.feishu.cn/document/server-docs/mail-v1/mail-group/mailgroup-permission_member/batch_create
...
名称 | 类型 | 必填 | 描述
---|---|---|---
items | mailgroup.permission_member\[\] | 否 | 本次添加的邮件组权限成员列表<br>**数据校验规则**：<br>- 长度范围：`1` ～ `200`
permission_member_id | string | 否 | 权限组内成员唯一标识（在请求体中不用填）<br>**示例值**："xxxxxxxxxxxxxxx"
...
email | string | 否 | 成员邮箱地址（当成员类型是MAIL_GROUP/PUBLIC_MAILBOX时有值）<br>**示例值**："xxx@xx.x"
type | string | 否 | 成员类型<br>**示例值**："USER"<br>**可选值有**：<br>- USER：内部用户<br>- DEPARTMENT：部门<br>- MAIL_GROUP：邮件组<br>- PUBLIC_MAILBOX：公共邮箱
...
type | string | 成员类型<br>**可选值有**：<br>- USER：内部用户<br>- DEPARTMENT：部门<br>- MAIL_GROUP：邮件组<br>- PUBLIC_MAILBOX：公共邮箱
