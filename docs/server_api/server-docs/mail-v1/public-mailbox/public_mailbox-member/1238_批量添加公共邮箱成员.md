...
> Source: https://open.feishu.cn/document/server-docs/mail-v1/public-mailbox/public_mailbox-member/batch_create
...
名称 | 类型 | 必填 | 描述
---|---|---|---
items | public_mailbox.member\[\] | 是 | 本次调用添加的公共邮箱成员列表<br>**数据校验规则**：<br>- 长度范围：`1` ～ `200`
member_id | string | 否 | 公共邮箱内成员唯一标识（在请求体中不用填）<br>**示例值**："xxxxxxxxxxxxxxx"
user_id | string | 否 | 租户内用户的唯一标识（当成员类型是USER时有值）<br>**示例值**："xxxxxxxxxx"
type | string | 否 | 成员类型<br>**示例值**："USER"<br>**可选值有**：<br>- USER：内部用户
...
type | string | 成员类型<br>**可选值有**：<br>- USER：内部用户
