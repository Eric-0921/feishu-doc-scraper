...
> Source: https://open.feishu.cn/document/server-docs/mail-v1/public-mailbox/public_mailbox/create
...
字段权限要求 | **注意事项**：该接口返回体中存在下列敏感字段，仅当开启对应的权限后才会返回；如果无需获取这些字段，则不建议申请<br>查看公共邮箱数据驻留地(mail:public_mailbox.public_mailbox_geo)
...
名称 | 类型 | 必填 | 描述
---|---|---|---
email | string | 否 | 公共邮箱地址<br>**示例值**："test_public_mailbox@xxx.xx"
name | string | 否 | 公共邮箱名称<br>**示例值**："test public mailbox"
geo | string | 否 | 数据驻留地<br>**示例值**："cn"
