...
> Source: https://open.feishu.cn/document/server-docs/approval-v4/instance/cc
...
user_id | string | 是 | 发起当前操作的用户 ID，ID 类型与查询参数 user_id_type 取值一致。<br>**示例值**："f7cb567e"
cc_user_ids | string\[\] | 是 | 抄送人的用户 ID 列表，ID 类型与查询参数 user_id_type 取值一致。<br>**示例值**：["f7cb567e"]
comment | string | 否 | 抄送留言<br>**示例值**："ok"
