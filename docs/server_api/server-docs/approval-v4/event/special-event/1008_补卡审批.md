...
基本 | &nbsp;
---|---
事件类型 | approval.instance.remedy_group_update_v4
...
employee_id | string | 审批发起人的 user_id。你可以调用[获取单个用户信息](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/user/get)接口，通过 user_id 获取用户信息。
...
start_time | int | 审批发起时间，秒级时间戳。
end_time | int | 审批结束时间，秒级时间戳。
remedy_time | long | 补卡时间，毫秒级时间戳。
remedy_reason | string | 补卡原因。
status | string | 审批实例状态。审批实例通过时取值为 `APPROVED`
type | string | 固定取值 `remedy_approval_v2`
