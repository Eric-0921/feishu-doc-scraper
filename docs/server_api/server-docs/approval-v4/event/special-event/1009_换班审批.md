...
> Source: https://open.feishu.cn/document/server-docs/approval-v4/event/special-event/shift-change
...
type | string | 事件类型，固定取值 `shift_approval`
...
employee_id | string | 审批提交人的 user_id。
open_id | string | 审批提交人的 open_id。
start_time | int | 审批发起时间，秒级时间戳。
end_time | int | 审批结束时间，秒级时间戳。
shift_time | string | 换班时间。示例格式 `2018-12-01 12:00:00`
return_time | string | 还班时间。示例格式 `2018-12-01 12:00:00`
shift_reason | string | 换班事由。
