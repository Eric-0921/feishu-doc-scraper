...
> Source: https://open.feishu.cn/document/server-docs/approval-v4/instance-comment/list
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 查看、创建、更新、删除审批应用相关信息(approval:approval)<br>访问审批应用(approval:approval:readonly)<br>查看、创建、更新、删除原生审批评论相关信息(approval:instance.comment)
...
page_size | int | 否 | 分页大小，用于限制一次请求返回的数据量上限。<br>**示例值**：10<br>**数据校验规则**：<br>- 最大值：`1000`
...
is_delete | int | 是否删除。可能值有：<br>- 0：未删除<br>- 1：已删除
...
name | string | 被艾特人的姓名
offset | string | 被艾特人在评论中的位置，从 0 开始。例如：<br>- 取值为 0 时的效果：@username 示例文本<br>- 取值为 2 时的效果：示例 @username 文本
commentator | string | 评论创建人
extra | string | 附加字段
