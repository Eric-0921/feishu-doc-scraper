...
> Source: https://open.feishu.cn/document/server-docs/corehr-v1/leave/leave_types
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 获取预置数据信息(corehr:common_data.preset_data:read)<br>获取核心人事信息(corehr:corehr:readonly)<br>获取休假信息(corehr:leave:read)<br>更新核心人事信息(corehr:corehr)
...
status | string | 否 | 假期类型状态（不传则为全部）<br>可选值有：<br>- 1：已启用<br>- 2：已停用<br>**示例值**：1
...
status | int | 假期类型状态<br>可选值有：<br>- 1：已启用<br>- 2：已停用
