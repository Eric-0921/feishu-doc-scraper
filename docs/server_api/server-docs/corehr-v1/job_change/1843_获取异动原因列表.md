...
> Source: https://open.feishu.cn/document/server-docs/corehr-v1/job_change/query-2
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 获取预置数据信息(corehr:common_data.preset_data:read)<br>获取核心人事信息(corehr:corehr:readonly)<br>获取异动原因信息(corehr:transfer_reason:read)<br>更新预置数据信息(corehr:common_data.preset_data:write)<br>更新核心人事信息(corehr:corehr)
...
名称 | 类型 | 必填 | 描述
---|---|---|---
active | boolean | 否 | 异动原因状态<br>**示例值**：true
transfer_reason_unique_identifier | string\[\] | 否 | 异动原因唯一标识，多条时最多数量为10<br>**示例值**：voluntary_transfer<br>**数据校验规则**：<br>- 最大长度：`10`
