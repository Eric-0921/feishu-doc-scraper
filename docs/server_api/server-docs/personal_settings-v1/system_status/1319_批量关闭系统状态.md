...
> Source: https://open.feishu.cn/document/server-docs/personal_settings-v1/system_status/batch_close
...
名称 | 类型 | 描述
---|---|---
system_status_id | string | 系统状态ID<br>[获取系统状态ID](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/personal_settings-v1/system_status/list)<br>**示例值**："7101214603622940671"
...
名称 | 类型 | 必填 | 描述
---|---|---|---
user_list | string\[\] | 是 | 用户ID列表，传入的ID类型由user_id_type决定，推荐使用 OpenID，获取方式可参考文档[如何获取 Open ID](https://open.feishu.cn/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-openid)。<br>**示例值**：["ou_53edd3282dbc2fdbe5c593cfa5ce82ab"]<br>**数据校验规则**：<br>- 长度范围：`1` ～ `50`
...
user_id | string | 用户ID
result | string | 关闭结果<br>**可选值有**：<br>- success：成功<br>- fail：失败<br>- invisible_user_id：用户ID不可见<br>- invalid_user_id：用户ID无效<br>- resign_user_id：用户离职
