...
> Source: https://open.feishu.cn/document/server-docs/attendance-v1/user_setting/query
...
支持的应用类型 | Custom App
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用** | 导出打卡管理规则(attendance:rule:readonly)
...
名称 | 类型 | 必填 | 描述
---|---|---|---
user_ids | string\[\] | 是 | employee_no 或 employee_id 列表，对应employee_type<br>**示例值**：["abd754f7"]<br>**数据校验规则**：<br>- 最大长度：`100`
