...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 更新日历及日程信息(calendar:calendar)<br>生成 CalDAV 配置信息(calendar:settings.caldav:create)
...
名称 | 类型 | 必填 | 描述
---|---|---|---
device_name | string | 否 | 需要同步日历的设备名，在日历中用来展示。<br>**默认值**：空<br>**示例值**："iPhone"<br>**数据校验规则**：<br>- 最大长度：`100` 字符
