...
page_size | int | 否 | 一次请求返回的最大日历数量。<br>**示例值**：10<br>**默认值**：`20`<br>**数据校验规则**：<br>- 最大值：`50`

### 请求体

名称 | 类型 | 必填 | 描述
---|---|---|---
query | string | 是 | 搜索关键字。接口将会搜索标题或描述中包含该关键字的公共日历或用户主日历。<br>**示例值**："query words"<br>**数据校验规则**：<br>- 长度范围：`1` ～ `200` 字符
