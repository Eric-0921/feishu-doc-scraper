...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 更新日历及日程信息(calendar:calendar)<br>读取日历访问控制权限(calendar:calendar.acl:read)<br>获取日历、日程及忙闲信息(calendar:calendar:readonly)
...
page_size | int | 否 | 分页大小，即一次请求返回的最大条目数。<br>**注意**：最小值 10，即取值小于 10 时统一按 10 处理。<br>**示例值**：10<br>**默认值**：`20`<br>**数据校验规则**：<br>- 最大值：`50`
