...
> Source: https://open.feishu.cn/document/server-docs/docs/bitable-v1/app-dashboard/list
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 获取仪表盘信息(base:dashboard:read)<br>查看、评论、编辑和管理多维表格(bitable:app)<br>查看、评论和导出多维表格(bitable:app:readonly)
...
名称 | 类型 | 必填 | 描述
---|---|---|---
page_size | int | 否 | 分页大小<br>**示例值**：10<br>**数据校验规则**：<br>- 最大值：`500`
page_token | string | 否 | 分页标记，第一次请求不填，表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token，下次遍历可采用该 page_token 获取查询结果<br>**示例值**：blknkqrP3RqUkcAW
...
msg | string | 错误描述
data | \- | \-
dashboards | app.dashboard\[\] | 仪表盘信息
block_id | string | 仪表盘 ID
name | string | 仪表盘名称
...
400 | 1254011 | The page_size must be between 0 and 100. | page_size参数非法
