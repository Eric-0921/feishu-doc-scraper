...
> Source: https://open.feishu.cn/document/server-docs/docs/bitable-v1/app-dashboard/copy
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 复制仪表盘(base:dashboard:copy)<br>查看、评论、编辑和管理多维表格(bitable:app)
...
msg | string | 错误描述
data | \- | \-
block_id | string | 新的仪表盘的 block_id
name | string | 新的仪表盘名称
