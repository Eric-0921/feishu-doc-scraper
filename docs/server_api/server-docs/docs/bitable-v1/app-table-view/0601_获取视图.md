...
> Source: https://open.feishu.cn/document/server-docs/docs/bitable-v1/app-table-view/get
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 检索视图(base:view:read)<br>查看、评论、编辑和管理多维表格(bitable:app)<br>查看、评论和导出多维表格(bitable:app:readonly)
...
view_type | string | 视图类型<br>**可选值有**：<br>- `grid`：表格视图<br>- `kanban`：看板视图<br>- `gallery`：画册视图<br>- `gantt`：甘特视图<br>- `form`：表单视图
