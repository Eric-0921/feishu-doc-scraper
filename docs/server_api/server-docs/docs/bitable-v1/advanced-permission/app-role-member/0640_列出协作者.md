...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 列出协作者(base:collaborator:read)<br>查看、评论、编辑和管理多维表格(bitable:app)<br>查看、评论和导出多维表格(bitable:app:readonly)
...
名称 | 类型 | 必填 | 描述
---|---|---|---
page_size | int | 否 | 分页大小<br>**示例值**：100<br>**数据校验规则**：<br>- 最大值：`100`
...
member_type | string | 协作者的类型<br>**可选值有**：<br>- user：用户<br>- chat：群组<br>- department：部门
