...
> Source: https://open.feishu.cn/document/server-docs/docs/bitable-v1/app-table/batch_delete
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 删除数据表(base:table:delete)<br>查看、评论、编辑和管理多维表格(bitable:app)
