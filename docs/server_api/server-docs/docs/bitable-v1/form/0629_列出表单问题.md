...
> Source: https://open.feishu.cn/document/server-docs/docs/bitable-v1/form/list
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 获取表单数据(base:form:read)<br>查看、评论、编辑和管理多维表格(bitable:app)<br>查看、评论和导出多维表格(bitable:app:readonly)
...
page_token | string | 否 | 分页标记，第一次请求不填，表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token，下次遍历可采用该 page_token 获取查询结果<br>**示例值**：vewTpR1urY
...
msg | string | 错误描述
data | \- | \-
items | app.table.form.field\[\] | 表单问题信息
field_id | string | 表单问题 ID
title | string | 表单问题
description | string | 问题描述
required | boolean | 该问题是否必填。可选值：<br>- true：必填<br>- false：非必填
visible | boolean | 该问题是否可见。可选值：<br>- true：可见<br>- false：不可见
...
400 | 1254200 | InternalError | 内部错误，请联系[技术支持](https://applink.feishu.cn/TLJpeNdW)
