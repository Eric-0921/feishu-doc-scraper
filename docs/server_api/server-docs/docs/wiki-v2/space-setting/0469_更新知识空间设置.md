...
> Source: https://open.feishu.cn/document/server-docs/docs/wiki-v2/space-setting/update
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 更新知识空间设置(wiki:setting:write_only)<br>查看、编辑和管理知识库(wiki:wiki)
...
名称 | 类型 | 描述
---|---|---
space_id | string | 知识空间id<br>**示例值**："1565676577122621"

### 请求体

名称 | 类型 | 必填 | 描述
---|---|---|---
create_setting | string | 否 | 谁可以创建空间的一级页面： "admin_and_member" = 管理员和成员 "admin"  - 仅管理员<br>**示例值**："admin/admin_and_member"
security_setting | string | 否 | 可阅读用户可否创建副本/打印/导出/复制： "allow" - 允许 "not_allow" - 不允许<br>**示例值**："allow/not_allow"
comment_setting | string | 否 | 可阅读用户可否评论： "allow" - 允许 "not_allow" - 不允许<br>**示例值**："allow/not_allow"
