...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 查看、评论、编辑和管理云空间中所有文件(drive:drive)<br>查看、评论、编辑和管理电子表格(sheets:spreadsheet)<br>创建电子表格(sheets:spreadsheet:create)
...
名称 | 类型 | 必填 | 描述
---|---|---|---
title | string | 否 | 表格标题<br>**示例值**："Sales sheet"<br>**数据校验规则**：<br>- 长度范围：`0` ～ `255` 字符
