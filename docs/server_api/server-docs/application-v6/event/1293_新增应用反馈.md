...
> Source: https://open.feishu.cn/document/server-docs/application-v6/event/created-2
...
基本 | &nbsp;
---|---
事件类型 | application.application.feedback.created_v6
...
app_id | string | 被反馈应用 ID
feedback_time | string | 反馈提交时间，格式为yyyy-mm-dd hh:mm:ss
tenant_name | string | 反馈用户的租户名
feedback_type | int | 反馈类型（枚举值，1：故障反馈，2：产品建议）
...
contact | string | 用户填写的联系方式<br>**字段权限要求（满足任一）**：<br>获取用户邮箱信息(contact:user.email:readonly)<br>获取用户手机号(contact:user.phone:readonly)
description | string | 反馈详情
