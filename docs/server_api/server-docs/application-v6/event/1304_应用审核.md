...
> Source: https://open.feishu.cn/document/server-docs/application-v6/event/audit
...
基本 | &nbsp;
---|---
事件类型 | application.application.app_version.audit_v6
支持的应用类型 | Custom App
...
event | \- | \-
operator_id | user_id | 通过 / 拒绝应用审核的管理员 id
...
version_id | string | 被审核的应用版本 id
creator_id | user_id | 应用创建者的 id
union_id | string | 用户的 union id
user_id | string | 用户的 user id<br>**字段权限要求**：<br>获取用户 user ID(contact:user.employee_id:readonly)
open_id | string | 用户的 open id
app_id | string | 审核应用的 id
operation | string | 审核通过 / 拒绝<br>**可选值有**：<br>- audited：审核通过<br>- reject：审核拒绝
remark | string | 审核信息，当审核拒绝时，管理员填写的拒绝理由
audit_source | string | 应用审核的方式<br>**可选值有**：<br>- administrator：管理员审核<br>- auto：自动免审
