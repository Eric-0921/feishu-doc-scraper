...
> Source: https://open.feishu.cn/document/server-docs/application-v6/event/app-first-enabled

---

# 首次启用应用

当前租户首次安装并启用应用时，触发当前事件。启用应用包含以下场景：
...
{ 
    ...
    "event": { 
        ...
        "installer": {	// 租户管理员直接安装应用、租户普通成员申请安装应用时，返回此字段。
             "open_id":"xxx" ,  // 用户对此应用的唯一标识，同一用户对不同应用的open_id不同
        },
        "installer_employee": {  // 租户普通成员免审安装应用时，返回此字段。
             "open_id":"xxx" ,  // 用户对此应用的唯一标识，同一用户对不同应用的open_id不同
		}
    } 
}
...
func main() {
	...
	eventHandler := dispatcher.NewEventDispatcher("", "").
		OnCustomizedEvent("app_open", func(ctx context.Context, event *larkevent.EventReq) error {
...
event_handler = lark.EventDispatcherHandler.builder("", "") \
    .register_p1_customized_event("app_open", do_customized_event) \
    .build()
...
public class Sample {
    ...
    private static final EventDispatcher EVENT_HANDLER = EventDispatcher.newBuilder("", "")
            .onCustomizedEvent("app_open", new CustomEventHandler() {
                @Override
