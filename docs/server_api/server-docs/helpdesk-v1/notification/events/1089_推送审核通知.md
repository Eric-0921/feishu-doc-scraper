...
> Source: https://open.feishu.cn/document/server-docs/helpdesk-v1/notification/events/approve
...
基本 | &nbsp;
---|---
事件类型 | helpdesk.notification.approve_v1
支持的应用类型 | Custom App
...
event | \- | \-
notification_id | string | 推送任务唯一ID
helpdesk_id | string | 服务台唯一ID
approve_status | string | REJECTED(审核不通过)<br>APPROVED(审核通过)<br>CANCELED(取消审核)<br>DELETED(删除审核)
