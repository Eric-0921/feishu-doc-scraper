...
> Source: https://open.feishu.cn/document/server-docs/helpdesk-v1/ticket-management/ticket_customized_field/list-ticket-customized-fields
...
名称 | 类型 | 必填 | 描述
---|---|---|---
page_token | string | 否 | 分页标记，第一次请求不填，表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token，下次遍历可采用该 page_token 获取查询结果<br>**示例值**："6948728206392295444"
page_size | int | 否 | 分页大小<br>**示例值**：10；默认为20<br>**数据校验规则**：<br>- 最大值：`100`

### 请求体

名称 | 类型 | 必填 | 描述
---|---|---|---
visible | boolean | 否 | 是否可见<br>**示例值**：true
