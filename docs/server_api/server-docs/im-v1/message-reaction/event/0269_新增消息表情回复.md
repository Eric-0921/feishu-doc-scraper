...
> Source: https://open.feishu.cn/document/server-docs/im-v1/message-reaction/event/created
...
权限要求<br>**订阅该事件所需的权限，开启其中任意一项权限即可订阅**<br>开启任一权限即可 | 查看消息表情回复(im:message.reactions:read)<br>获取单聊、群组消息(im:message:readonly)
...
operator_type | string | 操作人类型。可能值有：<br>- user：用户，此时 user_id 参数有返回值。<br>- app：应用，此时 app_id 参数有返回值。
...
action_time | string | 添加表情回复的时间戳。单位：ms
...
func main() {
	...
	eventHandler := dispatcher.NewEventDispatcher("", "").
		OnP2MessageReactionCreatedV1(func(ctx context.Context, event *larkim.P2MessageReactionCreatedV1) error {
			fmt.Printf("[ OnP2MessageReactionCreatedV1 access ], data: %s\n", larkcore.Prettify(event))
			return nil
		})
...
def do_p2_im_message_reaction_created_v1(data: lark.im.v1.P2ImMessageReactionCreatedV1) -> None:
    print(f'[ do_p2_im_message_reaction_created_v1 access ], data: {lark.JSON.marshal(data, indent=4)}')
    ...
    private static final EventDispatcher EVENT_HANDLER = EventDispatcher.newBuilder("", "")
            .onP2MessageReactionCreatedV1(new ImService.P2MessageReactionCreatedV1Handler() {
                ...
                public void handle(P2MessageReactionCreatedV1 event) throws Exception {
                    System.out.printf("[ onP2MessageReactionCreatedV1 access ], data: %s\n", Jsons.DEFAULT.toJson(event.getEvent()));
                }
            })
            .build();
