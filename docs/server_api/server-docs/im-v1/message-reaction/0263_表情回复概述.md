...
operator_id | string | 操作人 ID，具体的取值与 `operator_type` 相关：<br>- 当 `operator_type` 取值 `app` 时返回机器人所属的应用 ID（App ID）。<br>- 当 `operator_type` 取值 `user` 时返回用户 ID（user_id、open_id 或者 union_id）。
