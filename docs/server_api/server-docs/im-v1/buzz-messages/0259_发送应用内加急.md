...
> Source: https://open.feishu.cn/document/server-docs/im-v1/buzz-messages/urgent_app
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 发送应用内加急消息(im:message.urgent)<br>发送应用内加急消息（历史版本）(im:message.urgent:app_send)
