...
> Source: https://open.feishu.cn/document/server-docs/historic-version//event/scope-change

---

# 授权范围变更

**注意事项**：为了更好地提升该事件的安全性，我们对其进行了升级，请尽快迁移至[新版本>>](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/scope/events/updated)
...
![图片名称](//sf3-cn.feishucdn.com/obj/website-img/9e6699c282c5964d7e6aba0fca6f14ae_5ysPlHAjBY.png)
当此范围变化时，就会触发授权范围变化事件。
- 依赖权限：==以应用身份访问通讯录==
- 特殊说明：此事件无需订阅，开通了==以应用身份访问通讯录== 权限后自动订阅。
- 搭配使用：[获取通讯录授权范围](https://open.feishu.cn/document/ukTMukTMukTM/ugjNz4CO2MjL4YzM)
...
func main() {
	...
	eventHandler := dispatcher.NewEventDispatcher("", "").
		OnCustomizedEvent("contact_scope_change", func(ctx context.Context, event *larkevent.EventReq) error {
...
event_handler = lark.EventDispatcherHandler.builder("", "") \
    .register_p1_customized_event("contact_scope_change", do_customized_event) \
    .build()
...
public class Sample {
    ...
    private static final EventDispatcher EVENT_HANDLER = EventDispatcher.newBuilder("", "")
            .onCustomizedEvent("contact_scope_change", new CustomEventHandler() {
                @Override
...
wsClient.start({
    ...
    eventDispatcher: new Lark.EventDispatcher({}).register({
        'contact_scope_change': async (data) => {
            console.log(data);
        }
    })
});
