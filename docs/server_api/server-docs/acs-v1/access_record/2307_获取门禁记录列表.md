...
> Source: https://open.feishu.cn/document/server-docs/acs-v1/access_record/list
...
HTTP Method | GET
支持的应用类型 | Custom App
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用** | 查看智能门禁记录(acs:access_record:readonly)
...
名称 | 类型 | 必填 | 描述
---|---|---|---
page_size | int | 否 | 分页大小<br>**示例值**：100<br>**默认值**：`100`<br>**数据校验规则**：<br>- 最大值：`500`
...
from | int | 是 | 记录开始时间，单位秒<br>**示例值**：1624520521
to | int | 是 | 记录结束时间，单位秒，<br>时间跨度不能超过30天<br>**示例值**：1624520521
device_id | string | 否 | 门禁设备 ID<br>**示例值**："7091146989218002577"
...
access_type | string | 识别方式<br>**可选值有**：<br>- FA：人脸识别方式
