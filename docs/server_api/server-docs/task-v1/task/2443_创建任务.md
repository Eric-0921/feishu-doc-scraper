...
> Source: https://open.feishu.cn/document/server-docs/task-v1/task/create
...
summary | string | 任务的标题，类型为文本字符串。<br>如果要在任务标题中插入 URL 或者 @某个用户，请使用rich_summary字段。<br>创建任务时，任务标题(summary字段)和任务富文本标题(rich_summary字段)不能同时为空，需要至少填充其中一个字段。<br><md-alert><br>任务标题和任务富文本标题同时存在时只使用富文本标题。
description | string | 任务的描述，类型为文本字符串。<br>如果要在任务描述中插入 URL 或者 @某个用户，请使用rich_description字段。<br>**注意事项**：
