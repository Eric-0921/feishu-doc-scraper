...
> Source: https://open.feishu.cn/document/server-docs/task-v1/task-comment/get
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 查看、创建、修改、删除任务评论（历史版本）(task:comment)<br>查看任务评论（历史版本）(task:comment:readonly)
...
comment_id | string | 评论ID<br>**示例值**："6937231762296684564"
