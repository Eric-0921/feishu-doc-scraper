...
> Source: https://open.feishu.cn/document/server-docs/task-v1/task-comment/events/updated
...
基本 | &nbsp;
---|---
事件类型 | task.task.comment.updated_v1
支持的应用类型 | Custom App、Store App
权限要求<br>**订阅该事件所需的权限，开启其中任意一项权限即可订阅** | 查看、创建、修改和删除任务评论(task:comment)
...
event | \- | \-
task_id | string | 任务ID
comment_id | string | 任务评论ID
parent_id | string | 任务评论父ID
obj_type | int | 通知类型（1：创建评论，2：回复评论，3：更新评论，4：删除评论）
...
func main() {
	...
	eventHandler := dispatcher.NewEventDispatcher("", "").
		OnP2TaskCommentUpdatedV1(func(ctx context.Context, event *larktask.P2TaskCommentUpdatedV1) error {
			fmt.Printf("[ OnP2TaskCommentUpdatedV1 access ], data: %s\n", larkcore.Prettify(event))
			return nil
		})
...
def do_p2_task_task_comment_updated_v1(data: lark.task.v1.P2TaskTaskCommentUpdatedV1) -> None:
    print(f'[ do_p2_task_task_comment_updated_v1 access ], data: {lark.JSON.marshal(data, indent=4)}')

# 注册事件 Register event
event_handler = lark.EventDispatcherHandler.builder("", "") \
    .register_p2_task_task_comment_updated_v1(do_p2_task_task_comment_updated_v1) \
    .build()
    ...
    private static final EventDispatcher EVENT_HANDLER = EventDispatcher.newBuilder("", "")
            .onP2TaskCommentUpdatedV1(new TaskService.P2TaskCommentUpdatedV1Handler() {
                ...
                public void handle(P2TaskCommentUpdatedV1 event) throws Exception {
                    System.out.printf("[ onP2TaskCommentUpdatedV1 access ], data: %s\n", Jsons.DEFAULT.toJson(event.getEvent()));
                }
            })
            .build();
