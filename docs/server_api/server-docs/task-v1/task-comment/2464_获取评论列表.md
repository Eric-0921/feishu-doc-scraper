...
> Source: https://open.feishu.cn/document/server-docs/task-v1/task-comment/list
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 查看、创建、编辑和删除飞书任务（历史版本）(task:task)<br>查看任务详情(task:task:readonly)
...
名称 | 类型 | 描述
---|---|---
task_id | string | 任务id<br>**示例值**：""83912691-2e43-47fc-94a4-d512e03984fa""
...
list_direction | int | 否 | 评论排序标记，可按照评论时间从小到大查询，或者评论时间从大到小查询，不填默认按照从小到大<br>**示例值**：0<br>**可选值有**：<br>- 0：按照回复时间从小到大查询<br>- 1：按照回复时间从大到小查询<br>**默认值**：`0`
...
content | string | 评论内容。<br><md-alert><br>评论内容和富文本评论内容同时存在时只使用富文本评论内容。
