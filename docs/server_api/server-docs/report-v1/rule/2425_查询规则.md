...
> Source: https://open.feishu.cn/document/server-docs/report-v1/rule/query
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用** | 查看汇报规则(report:rule:readonly)
...
include_deleted | int | 否 | 是否包括已删除，默认未删除<br>**示例值**：0<br>**可选值有**：<br>- 0：不包括已删除<br>- 1：包括已删除<br>**数据校验规则**：<br>- 取值范围：`0` ～ `1`
...
name | string | 字段名称
type | string | 字段类型<br>**可选值有**：<br>- text：文本<br>- number：数字<br>- dropdown：单选<br>- image：图片<br>- attachement：附件<br>- multiSelect：多选<br>- address：地址<br>- datetime：时间
is_deleted | int | 规则是否已删除<br>**可选值有**：<br>- 0：未删除<br>- 1：已删除
