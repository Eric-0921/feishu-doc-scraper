...
> Source: https://open.feishu.cn/document/server-docs/report-v1/rule-view/remove
...
支持的应用类型 | Custom App
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用** | 获取汇报业务域的全部权限(report:report)
...
名称 | 类型 | 描述
---|---|---
rule_id | string | 汇报规则ID<br>**示例值**："6894419345318182122"<br>**数据校验规则**：<br>- 最小长度：`1` 字符
...
名称 | 类型 | 必填 | 描述
---|---|---|---
user_ids | string\[\] | 否 | 列表为空删除规则下全用户视图，列表不为空删除指定用户视图，大小限制200。<br>**示例值**：["ou_d6a5b5a55c77ca0b5b6c6ca0dd628c85"]<br>**数据校验规则**：<br>- 长度范围：`0` ～ `200`
