...
> Source: https://open.feishu.cn/document/server-docs/vc-v1/meeting/events/recording_ready
...
**注意事项**：收到该事件后，方可进行录制文件获取、授权等操作。

## 事件

基本 | &nbsp;
---|---
事件类型 | vc.meeting.recording_ready_v1
...
func main() {
	...
	eventHandler := dispatcher.NewEventDispatcher("", "").
		OnP2MeetingRecordingReadyV1(func(ctx context.Context, event *larkvc.P2MeetingRecordingReadyV1) error {
			fmt.Printf("[ OnP2MeetingRecordingReadyV1 access ], data: %s\n", larkcore.Prettify(event))
			return nil
		})
...
def do_p2_vc_meeting_recording_ready_v1(data: lark.vc.v1.P2VcMeetingRecordingReadyV1) -> None:
    print(f'[ do_p2_vc_meeting_recording_ready_v1 access ], data: {lark.JSON.marshal(data, indent=4)}')

# 注册事件 Register event
event_handler = lark.EventDispatcherHandler.builder("", "") \
    .register_p2_vc_meeting_recording_ready_v1(do_p2_vc_meeting_recording_ready_v1) \
    .build()
...
public class Sample {
    ...
    private static final EventDispatcher EVENT_HANDLER = EventDispatcher.newBuilder("", "")
            .onP2MeetingRecordingReadyV1(new VcService.P2MeetingRecordingReadyV1Handler() {
                @Override
                public void handle(P2MeetingRecordingReadyV1 event) throws Exception {
                    System.out.printf("[ onP2MeetingRecordingReadyV1 access ], data: %s\n", Jsons.DEFAULT.toJson(event.getEvent()));
                }
            })
            .build();
