...
> Source: https://open.feishu.cn/document/server-docs/vc-v1/scope_config/get-4
...
名称 | 类型 | 描述
---|---|---
reserve_config_id | string | 会议室或层级ID，可通过会议室或层级相关查询接口获取<br>**示例值**："omm_3c5dd7e09bac0c1758fcf9511bd1a771"

### 查询参数

名称 | 类型 | 必填 | 描述
---|---|---|---
scope_type | int | 是 | 1表示层级，2表示会议室<br>**示例值**：2<br>**数据校验规则**：<br>- 取值范围：`1` ～ `2`
...
msg | string | 错误描述
data | \- | \-
disable_inform | disable_inform_config | 会议室禁用通知配置
if_cover_child_scope | boolean | 是否覆盖子层级及会议室
if_inform | boolean | 禁用状态变更通知开关
informed_users | subscribe_user\[\] | 通知成员列表
user_id | string | 预定管理员ID
informed_depts | subscribe_department\[\] | 通知部门列表
