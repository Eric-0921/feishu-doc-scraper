...
> Source: https://open.feishu.cn/document/server-docs/minutes-v1/minute-statistics/get
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 查看、创建、编辑及管理妙记文件(minutes:minutes)<br>获取妙记的统计信息(minutes:minutes.statistics:read)<br>查看妙记文件(minutes:minutes:readonly)
...
名称 | 类型 | 描述
---|---|---
minute_token | string | 妙记唯一标识。可从妙记的 URL 链接中获取，一般为最后一串字符：https://sample.feishu.cn/minutes/==obcnq3b9jl72l83w4f14xxxx==<br>**示例值**："obcnq3b9jl72l83w4f14xxxx"<br>**数据校验规则**：<br>- 长度范围：`24` ～ `24` 字符
