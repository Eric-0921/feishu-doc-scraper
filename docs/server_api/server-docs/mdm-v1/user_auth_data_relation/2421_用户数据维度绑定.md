...
> Source: https://open.feishu.cn/document/server-docs/mdm-v1/user_auth_data_relation/bind
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用** | 绑定及解绑用户数据维度权限(mdm:user_auth)
...
名称 | 类型 | 必填 | 描述
---|---|---|---
root_dimension_type | string | 是 | 数据类型编码<br>**示例值**："gongsi"<br>**数据校验规则**：<br>- 长度范围：`1` ～ `200` 字符
sub_dimension_types | string\[\] | 是 | 数据编码列表<br>**示例值**：["code_dimension"]<br>**数据校验规则**：<br>- 长度范围：`1` ～ `200`
authorized_user_ids | string\[\] | 是 | 授权人的lark id<br>**示例值**：["ou_7dab8a3d3cdcc9da365777c7ad535d62"]<br>**数据校验规则**：<br>- 长度范围：`1` ～ `200`
uams_app_id | string | 是 | uams系统中应用id<br>**示例值**："uams-tenant-test"<br>**数据校验规则**：<br>- 长度范围：`1` ～ `200` 字符
