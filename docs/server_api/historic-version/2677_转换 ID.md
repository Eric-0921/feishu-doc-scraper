...
> Source: https://open.feishu.cn/document/historic-version/id_convert
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用** | 获取卡片信息(cardkit:card:read)
...
名称 | 类型 | 必填 | 描述
---|---|---|---
message_id | string | 是 | 消息 ID。通过[发送消息](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message/create)等接口获取。其消息类型（msg_type）需为卡片（interactive）<br>**示例值**："om_fbdf6ed2e17f1d98e78fb26c1370186e"<br>**数据校验规则**：<br>- 长度范围：`1` ～ `50` 字符
