...
支持的应用类型 | Custom App
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 删除部门(directory:department.delete:write)<br>创建、更新、删除部门(directory:department:write)
...
名称 | 类型 | 描述
---|---|---
department_id | string | 部门ID，与department_id_type类型保持一致<br>**示例值**："weasdqwe"<br>**数据校验规则**：<br>- 最大长度：`64` 字符
