...
> Source: https://open.feishu.cn/document/client-docs/bot-v3/events/menu
...
字段权限要求 | **注意事项**：该接口返回体中存在下列敏感字段，仅当开启对应的权限后才会返回；如果无需获取这些字段，则不建议申请<br>获取机器人自定义菜单操作人名称(application:application.bot.operator_name:readonly)<br>获取用户 user ID(contact:user.employee_id:readonly)
...
operator_name | string | 用户名称<br>**数据校验规则**：<br>- 长度范围：`1` ～ `100` 字符<br>**字段权限要求**：<br>获取机器人自定义菜单操作人名称(application:application.bot.operator_name:readonly)
...
event_key | string | 菜单事件的唯一标识<br>**数据校验规则**：<br>- 长度范围：`1` ～ `30` 字符
timestamp | int64 | 用户点击菜单时间<br>**数据校验规则**：<br>- 长度范围：`1` ～ `30`
