...
> Source: https://open.feishu.cn/document/hire-v1/get-candidates/import-external-system-information/import-external-interview-info/batch_query
...
支持的应用类型 | Custom App
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 更新外部投递(hire:external_application)<br>查看外部投递(hire:external_application:readonly)
...
page_size | int | 否 | 分页大小<br>**示例值**：10<br>**默认值**：`10`<br>**数据校验规则**：<br>- 取值范围：`1` ～ `20`
...
名称 | 类型 | 必填 | 描述
---|---|---|---
external_interview_id_list | string\[\] | 否 | 外部面试 ID 列表，当传此值时，仅以此值作为条件查询，其他查询条件不生效<br>**示例值**：["6960663240925956660"]<br>**数据校验规则**：<br>- 长度范围：`0` ～ `20`
