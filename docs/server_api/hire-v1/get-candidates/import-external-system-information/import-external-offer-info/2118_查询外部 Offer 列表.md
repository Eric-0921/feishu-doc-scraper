...
> Source: https://open.feishu.cn/document/hire-v1/get-candidates/import-external-system-information/import-external-offer-info/batch_query
...
支持的应用类型 | Custom App
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 更新外部 Offer(hire:external_offer)<br>查看外部 Offer(hire:external_offer:readonly)
...
名称 | 类型 | 必填 | 描述
---|---|---|---
external_application_id | string | 否 | 外部投递 ID，可通过[查询外部投递列表](https://open.feishu.cn/document/ukTMukTMukTM/uMzM1YjLzMTN24yMzUjN/hire-v1/external_application/list)接口获取<br>**示例值**：6960663240925956660
page_size | int | 否 | 分页大小<br>**示例值**：10<br>**默认值**：`10`<br>**数据校验规则**：<br>- 最大值：`20`
...
名称 | 类型 | 必填 | 描述
---|---|---|---
external_offer_id_list | string\[\] | 否 | 外部 Offer ID 列表，当传此值时，仅以此值作为条件查询，其他查询条件不生效<br>**示例值**：["6960663240925956660"]<br>**数据校验规则**：<br>- 长度范围：`0` ～ `20`
