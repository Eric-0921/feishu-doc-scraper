...
> Source: https://open.feishu.cn/document/corehr-v1/siganture/signature_file/terminate
...
支持的应用类型 | Custom App
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用** | 终止电子签文件(corehr:signature_file:terminate)
...
名称 | 类型 | 必填 | 描述
---|---|---|---
ids | string\[\] | 是 | 欲终止的电子签文件id列表，文件ID可以通过调用「获取电子签文件列表」接口获取<br>**示例值**：["7410063506638112300"]<br>**数据校验规则**：<br>- 长度范围：`0` ～ `100`
operator | string | 是 | 操作人ID，ID的格式取决于query传参中传入的user_id_type字段；ID获取方式见「查询参数」<br>**示例值**："7386593105085988xxx"
terminate_reason | string | 是 | 终止原因，不得超过300字符<br>**示例值**："请求流程发生变化，需要重新签署，因此终止签署流程中的文件"
