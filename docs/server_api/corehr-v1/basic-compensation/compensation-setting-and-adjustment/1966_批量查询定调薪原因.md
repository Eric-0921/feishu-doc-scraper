...
> Source: https://open.feishu.cn/document/corehr-v1/basic-compensation/compensation-setting-and-adjustment/list
...
支持的应用类型 | Custom App
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用** | 获取基础薪酬的调薪原因信息(corehr:compensation_change_reason:read)
...
page_token | string | 否 | 分页标记，第一次请求不填，表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token，下次遍历可采用该 page_token 获取查询结果<br>**示例值**：12314342
