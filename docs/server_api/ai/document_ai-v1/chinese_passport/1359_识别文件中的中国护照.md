...
> Source: https://open.feishu.cn/document/ai/document_ai-v1/chinese_passport/recognize
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用** | 识别中国护照(document_ai:chinese_passport:recognize)
...
名称 | 类型 | 必填 | 描述
---|---|---|---
file | file | 是 | 识别的中国护照源文件<br>**示例值**：file binary
...
msg | string | 错误描述
data | \- | \-
chinese_passport | chinese_passport | 中国护照信息
entities | chinese_passport_entity\[\] | 识别出的实体类型
type | string | 识别的字段种类<br>**可选值有**：<br>- full_name_cn：中文姓名<br>- full_name_en：英文格式姓名<br>- date_of_birth：出生日期<br>- date_of_expiry：有效期至<br>- place_of_issue：签发地点<br>- passport_number：护照号码
