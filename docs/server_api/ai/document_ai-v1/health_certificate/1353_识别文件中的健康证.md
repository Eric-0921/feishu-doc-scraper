...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用** | 识别健康证(document_ai:health_certificate:recognize)
...
名称 | 类型 | 必填 | 描述
---|---|---|---
file | file | 是 | 识别的健康证源文件<br>**示例值**：file binary
...
type | string | 识别的字段种类<br>**可选值有**：<br>- name：姓名<br>- issued_by：发证机关<br>- date_of_handling：办证日期<br>- date_of_issue：发证日期<br>- date_of_medical_examination：体检日期<br>- valid_date：有效日期<br>- other_date：其他日期
