...
> Source: https://open.feishu.cn/document/task-v2/task/remove_tasklist
...
如果任务不在清单中，接口将返回成功。
**注意事项**：需要清单的可编辑权限。详情见[清单功能概述](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/task-v2/tasklist/overview)中的“清单是如何鉴权的？“章节。

## 请求

基本 | &nbsp;
---|---
HTTP URL | https://open.feishu.cn/open-apis/task/v2/tasks/:task_guid/remove_tasklist
HTTP Method | POST
...
名称 | 类型 | 描述
---|---|---
task_guid | string | 要从清单移除的任务的全局唯一ID<br>**示例值**："d300a75f-c56a-4be9-80d1-e47653028ceb"
...
名称 | 类型 | 必填 | 描述
---|---|---|---
tasklist_guid | string | 是 | 要移除的清单的全局唯一ID<br>**示例值**："d300a75f-c56a-4be9-80d1-e47653028ceb"<br>**数据校验规则**：<br>- 最大长度：`100` 字符
...
HTTP状态码 | 错误码 | 描述 | 排查建议
---|---|---|---
400 | 1470400 | 请求参数错误，如传入的清单GUID不合法。 | 错误原因见返回的msg提示的信息。
404 | 1470404 | 任务数据不存在或者已删除。 | 确认要访问的任务数据是否存在或已删除。
500 | 1470500 | 服务器错误。 | 尝试重试调用。如持续失败，请联系支持人员进行反馈。
403 | 1470403 | 缺少任务移出清单的权限。 | 检查调用身份是否有清单的可编辑权限。详情见[清单功能概述](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/task-v2/tasklist/overview)中的“清单是如何鉴权的？“章节。
