...
> Source: https://open.feishu.cn/document/task-v2/comment/list
...
权限要求<br>**调用该 API 所需的权限。开启其中任意一项权限即可调用**<br>开启任一权限即可 | 查看任务评论(task:comment:read)<br>查看、创建、修改、删除任务评论(task:comment:write)
...
direction | string | 否 | 返回数据的排序方式。"asc"表示从最老到最新顺序返回；"desc"表示从最新到最老顺序返回。默认为"asc"。<br>**示例值**：asc<br>**可选值有**：<br>- asc：评论发表时间升序<br>- desc：评论发表时间降序<br>**默认值**：`asc`
